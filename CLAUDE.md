...
│                        # Logging is configured *before* `load_config()`, which the group also covers
...
├── markup.py            # bmlib's Markdown subset: the source gate, and the HTML and
│                        # text renderings. Imported by both db/ and gui/, so it depends
│                        # on neither — `is_markdown_source()` is what keeps the
│                        # conversion off the sources that never sent Markdown
...
tests/                         # Test suite (17 test modules)
...
markup.py ← db/operations.py (_row_to_paper flattens a Markdown title to text)
          ← gui/helpers.py    (format_abstract_html turns markers into tags)
...
### Abstracts and titles: two prose shapes, one gate

Since bmlib 0.8.0 the **PubMed** fetcher stores titles and abstracts as Markdown — `**LABEL:**` sections, `*em*` / `~sub~` / `^sup^`, and prose backslash-escaped over ``\ ` * ~ ^``. **No other fetcher does.** medRxiv, bioRxiv, Europe PMC and OpenAlex store prose exactly as the source sent it, unescaped.

`bmnews/markup.py` owns both the marker set and that distinction. Nothing converts without `is_markdown_source(paper["sources"])` agreeing, and that gate is not decoration: run over a non-Markdown source the converter does not find markup, it *invents* it. `CYP2C19*2 and CYP2C19*17`, `HLA-B*57:01`, `5.9*10(4)`, `~50-~60` and a `(*P<0.05, **P<0.01)` significance legend all hug non-whitespace on both sides — which is exactly the condition a delimiter must meet to pair, so CommonMark's flanking rule narrows the damage without closing it. Measured over a 4,214-abstract corpus, converting ungated changed 18 abstracts and improved none.

Two renderings, because the surfaces differ:

- **Abstracts** keep their markup. Only the GUI shows one, so `gui/helpers.py::format_abstract_html` converts markers to tags *after* `escape()` and takes `paper.sources`; omitting the argument converts nothing, which is the safe default a template override predating the gate still gets.
- **Titles** are flattened to text in `_row_to_paper()`. Every surface bmnews has renders a title as text — reading pane, card list, both email digests, both Matrix bodies — and the decode point is the only place that also reaches a user's overridden template and the title the relevance prompt sends to the LLM. Subscript flattens (`CO~2~` → `CO2`) but superscript keeps its caret, since `10^6^` → `106` is a different number.

A merged publication lists every source it was fed by, so the gate answers on "any source emits Markdown" — an unconverted PubMed abstract shows backslashes on every escaped line, whereas converting a medRxiv one only risks a pairing.
...
- **transparency** — one row per publication (`paper_id` PK, `REFERENCES publications(id) ON DELETE CASCADE`), holding `transparency_score`, `risk_level`, `attempts`, `result_json` (bmlib's whole `TransparencyResult.to_dict()`) and `analyzed_at`. `attempts` exists because bmlib's analyzer sets its "an API answered" flag only on an HTTP 200, so it reports UNREACHABLE both for a network outage AND for a paper indexed in none of its five APIs — the two are indistinguishable, so "retry every UNKNOWN" would re-query every unindexed preprint on every run forever. `get_transparency_candidates()` retries only while `attempts < TRANSPARENCY_MAX_ATTEMPTS` (3); `bmnews transparency --refresh` resets it to 1. The normal queue is ordered best-combined-score first, but a **refresh run is ordered by `analyzed_at ASC NULLS FIRST`**: it has no "not done yet" predicate to narrow it, so score order would hand back the identical top-`limit` papers on every run and never reach the rest of the corpus. `NULLS FIRST` is explicit because SQLite sorts NULLs first in `ASC` and PostgreSQL sorts them last. The table carries no secondary index — both queries that mention `risk_level` reach a row through `paper_id`.
...
| File | Coverage |
|---|---|
| `test_cli.py` | The CLI itself rather than any one command: an unhandled exception reported as `Error: <command> failed: …` with exit code 1 and no traceback, the traceback still logged with `exc_info`, all nine commands covered because the handling sits on the group, a malformed config reported the same way and **its** traceback still reaching a handler (which pins logging being configured before `load_config` can raise) — and what must survive the wrapper, namely a command's own `ctx.exit()` code, a `UsageError`'s exit 2, and an `Abort` |
| `test_config.py` | Config loading, TOML parsing, backward-compat defaults |
| `test_db.py` | All database operations, migrations, storing/dedup, filtering, tagging, digests, paper extras, digest selection filters, notification candidate selection and batch recording (including its rollback), the v3 → v4 data migration, migration 6's cache purge, migration 7's `transparency` table, transparency candidate selection (including a refresh run walking the corpus rather than redoing one batch, which pins the backend-divergent `NULLS FIRST`), NULL text columns decoding to strings, and a PubMed title's Markdown being flattened at decode while a medRxiv title's `*` is left as the content it is — **run against SQLite and PostgreSQL** |
...
| `test_docs.py` | The docs drift backstop — backticked repo paths exist in `docs/dev/`, **in this file, and in `bmnews/gui/CLAUDE.md`** (allowlist for worked examples like `newsource.py`; other repos and `/routes` skipped by prefix, `~` paths dropped by the token charset before the prefix list is reached), `database.md`'s migration table matches `MIGRATIONS`, **both** test listings — `testing.md`'s fenced block and this file's table and module count — match `tests/`, and the `bmnews, version X` sample output in `docs/user/installation.md` matches `__version__`. The path scan runs as three separately asserted groups, never one glob: `checked` is the no-vacuous-pass guard, so pooled, `docs/dev/`'s candidates would hold the total up while a `CLAUDE.md` stopped being scanned. `docs/user/` stays out — it yields zero candidates, and an empty group cannot satisfy that guard. Failure lines name their file repo-relative, so the two `index.md` files are told apart. Parsers pinned against fixture strings, and the path scan itself pinned against seeded drift, so no check can pass vacuously: a missing anchor, an unclosed fence and a scan that resolved nothing all fail. The **bmlib pin** is checked the same way `__version__` is, because it is hand-copied into three docs and PR #34 moved it in five places at once: every listed file must name the pinned tag, and must name *a* tag — an earlier pattern anchored on the full URL matched neither `CLAUDE.md`'s elided `git+…@v0.9.1` nor HANDOVER.md's bare `@v0.9.1`, so seeded drift passed |
...
| `test_gui_helpers.py` | Abstract HTML formatting, over both shapes an abstract arrives in — the legacy HTML one (`<h4>` headings, mixed tags) and bmlib ≥ 0.8.0's Markdown (`**LABEL:**` sections, `*em*`/`~sub~`/`^sup^`, backslash-escaped prose). The **source gate** gets a class of its own: nine shapes that occur in real medRxiv/Europe PMC prose and that the flanking rule does *not* stop (star alleles, HLA types, scientific notation, UTR regions, `~50-~60`, a `(*P<0.05, **P<0.01)` legend) must survive escaped but unconverted, and omitting `sources` must convert nothing. Plus the three mechanisms whose absence the old suite could not see — the escape hold/restore pinned where flanking cannot stand in for it, `***x***` nesting rather than crossing its tags, and the NUL sentinel competing with a real escape — and a raising converter degrading to escaped text with the failure logged |
